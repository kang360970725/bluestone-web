<template>
  <div class="block" style="padding: 10px;">
    <div v-show="editData">
      <h3 style="text-align: center;">合计分红BTC数量：{{countPrice.toFixed(8)}}</h3>
          ...
          <div class="block-content" style="min-height: 512px;">
            <table class="table table-striped table-bordered">
              ...
              <tr v-for="item in dataList" :key='item.id'>
                ...
                <td>{{item.price}}</td>
                <td>
                  <a class="tile-image tile-image-padding tile-image-hover-grayscale preview" :data-preview-image="initUrl + '' +item.credential" data-preview-size="modal-lg">
                ...
                <td v-show="item.type == 2">
                  <div>
                    <button class="btn btn-default btn-xs">转账已确认</button>
                  </div>
                </td>
                <td v-show="item.type == 3">
                  <div>
                    <button class="btn btn-default btn-xs">无效申请</button>
                  </div>
                </td>
                <td>
                  <div v-show="item.type == 1">
                    <button class="btn btn-danger btn-xs" @click="showPromp(item)">驳回</button>
                    <button class="btn btn-success btn-xs" @click="setUserTypeFn(item,'2')">确认凭证有效</button>
                  </div>
                </td>
              </tr>
  ...
  export default {
    ...
    methods: {
      getInitData: function () {
        let _this = this;
        _this.$axios.get('getBonusList?pageIndex=' + _this.pagination.pageIndex + '&pageSize=' + _this.pagination.pageSize + '&type=' + _this.type).then(function (result) {
          if (result.status == 0) {
        ...
        //获取充值统计
        _this.$axios.get('countBonusSum').then(function (result) {
          if (result.status == 0) {
      ...
      modalResponse(json) {//驳回
        ...
        if (json.type === 'close') {
          ...
          _this.$axios.put('setBonusType', obj).then(function (result) {
            if (result.status == 0) {
      ...
      setUserTypeFn: function (item, type) {//确认通过
        let _this = this;
        if (confirm('确定当前操作?')) {
          let obj = {
            id: item.id,
